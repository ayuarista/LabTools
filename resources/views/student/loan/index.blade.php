                                ...
                                <td class="px-4 py-3 text-end whitespace-nowrap space-x-2">
                                    @if ($loan->status === 'approved')
                                        <form action="{{ route('loans.return.request', $loan->id) }}" method="POST"
                                            class="inline">
                                            @csrf
                                            <flux:button variant="primary" size="sm" type="submit" class="hover:cursor-pointer">Kembalikan</flux:button>
                                        </form>
                                    @endif
                                    ...
                                    @if ($loan->status === 'pending')
                                        <form action="{{ route('loans.edit', $loan->id) }}" method="POST"
                                            ...
                                            @method('PATCH')
                                            <button type="submit"
                                                class="bg-amber-100 text-amber-700 rounded-md px-3 py-1 text-sm font-medium">Edit</button>
                                        </form>
                                        <form action="{{ route('loans.cancel', $loan->id) }}" method="POST"
                                            ...
                                            @method('PATCH')
                                            <button type="submit"
                                                class="bg-red-100 text-red-700 rounded-md px-3 py-1 text-sm font-medium">Batalkan</button>
                                        </form>
                                    @endif
                                </td>
                            </tr>
